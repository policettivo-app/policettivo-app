...
export default async function handler(req, res) {
  ...
  if (tipo === 'log_firma') return handleLogFirma(req, res)

  return res.status(400).json({ error: 'Parametro tipo non valido (atteso: posturale | visita | video | log_firma)' })
}
...
// ════════════════════════════════════════════════════════════════════════════
// LOG_FIRMA  (audit dedicato per FIRMA consenso — bypass RLS su audit_log
// via service_key, con verifica identita' del chiamante)
// ════════════════════════════════════════════════════════════════════════════

async function handleLogFirma(req, res) {
  const token = (req.headers.authorization || '').replace('Bearer ', '').trim()
  if (!token) return res.status(401).json({ error: 'Token mancante' })

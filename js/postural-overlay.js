...
const LM = { shoulderL:11, shoulderR:12, hipL:23, hipR:24, kneeL:25, kneeR:26, ankleL:27, ankleR:28, ear:7 };
...
function _buildFrontal(lms,W,H){
  ...
  _chain([sMid,hMid], 8, COL.spine, 3).forEach(p=>lines.push(p));   // catena reale bianca
  [LM.shoulderL,LM.shoulderR,LM.hipL,LM.hipR,LM.kneeL,LM.kneeR,LM.ankleL,LM.ankleR]
    .forEach(i=>{const p=_pt(lms,i,W,H);lines.push({type:'point',x:p.x,y:p.y,color:COL.punto,r:6});});
  lines.push({type:'line',x1:sL.x,y1:sL.y,x2:sR.x,y2:sR.y,color:COL.reale,lw:3.5});
  lines.push({type:'line',x1:hL.x,y1:hL.y,x2:hR.x,y2:hR.y,color:COL.reale,lw:3.5});
  ...
  // catena VERDE ideale: stessa altezza spalle->bacino, ma sull'asse di gravita (simmetria)
  _chain([{x:footX,y:sMid.y},{x:footX,y:hMid.y}], 8, COL.spineIdeal, 3).forEach(p=>ideal.push(p));

  return { lines, idealLines:ideal, angles:{ spalle:_devFromHoriz(sL,sR), bacino:_devFromHoriz(hL,hR) } };
}
...
function _buildSagittal(lms,W,H){
  ...
  [ear,sh,hip,knee,ankle].forEach(p=>lines.push({type:'point',x:p.x,y:p.y,color:COL.punto,r:6}));
